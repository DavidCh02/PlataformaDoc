...
const props = defineProps({
    ...
    // URL de descarga alternativa (p. ej. binario de una versión histórica).
    ...
    <Teleport to="body">
        ...
        <section class="flex h-[96vh] w-full max-w-6xl flex-col overflow-hidden rounded-lg bg-white dark:bg-slate-800 shadow-2xl">
            <header class="flex flex-wrap items-center justify-between gap-3 border-b border-slate-200 dark:border-slate-700 px-4 py-3 sm:px-6">
                <div class="min-w-0">
                    <h2 class="truncate font-semibold text-slate-900 dark:text-white">{{ file.original_name }}</h2>
                    <p class="text-xs text-slate-500 dark:text-slate-400">Vista previa del archivo original</p>
                </div>
                <div class="flex items-center gap-2">
                    <div class="flex items-center gap-1 rounded-md border border-slate-300 dark:border-slate-600 px-1 py-1">
                        <button type="button" class="rounded px-2 py-1 text-sm text-slate-700 dark:text-slate-300 hover:bg-slate-100 dark:hover:bg-slate-700/60" title="Alejar" @click="zoomOut">−</button>
                        <button type="button" class="min-w-10 rounded px-2 py-1 text-center text-sm font-medium text-slate-700 dark:text-slate-300 hover:bg-slate-100 dark:hover:bg-slate-700/60" title="Restablecer zoom" @click="resetZoom">{{ zoomLevel }}%</button>
                        <button type="button" class="rounded px-2 py-1 text-sm text-slate-700 dark:text-slate-300 hover:bg-slate-100 dark:hover:bg-slate-700/60" title="Acercar" @click="zoomIn">+</button>
                    </div>
                    <a :href="downloadUrl || route('files.download', file.id)" class="inline-flex items-center gap-1 rounded-md border border-slate-300 dark:border-slate-600 px-3 py-2 text-sm font-medium text-slate-700 dark:text-slate-300 hover:bg-slate-50 dark:hover:bg-slate-700/40">Descargar original</a>
                    ...
                    <button type="button" class="rounded-md border border-slate-300 dark:border-slate-600 px-3 py-2 text-sm text-slate-700 dark:text-slate-300 hover:bg-slate-50 dark:hover:bg-slate-700/40" @click="emit('close')">Cerrar</button>
                </div>
            </header>
            <div class="docx-preview-scroll">
                <p v-if="loading" class="py-16 text-center text-sm text-slate-500 dark:text-slate-400">Cargando vista previa...</p>
...
<style>
.docx-preview-scroll { min-height: 0; flex: 1; overflow: auto; background: var(--pd-preview); }
.docx-wrapper-wrapper > section.docx-wrapper, .docx-preview-scroll section.docx-wrapper { box-shadow: var(--pd-shadow-lg) !important; margin-bottom: 20px !important; }
