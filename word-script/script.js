...
// ============================================================
// IMPORTANTE: ngrok Free asigna una URL nueva cada vez que se reinicia.
// Verifica la URL activa con: http://127.0.0.1:4040/api/tunnels
var BASE_URL = "https://42a2-190-155-116-255.ngrok-free.app/PlataformaDoc/public/api/addin";
...
function initAddIn() {
    ...
    function detectActiveEditSession(quiet) {
        ...
        return readPlataformaDocId().then(function (propId) {
            ...
            return api(API.session(propId)).then(function (res) {
                ...
                if (metadataSessionMode) exitCheckinView(false);
                if (!quiet) {
                    setStatus('No se pudo verificar la sesión: ' + (e.message || 'error desconocido') + '. Verifica BASE_URL (túnel ngrok) y que hayas iniciado sesión.', 'err');
                }
                return null;
            });
    ...
    function logout() {
        ...
        if (authToken) {
            var lheaders = { 'Accept': 'application/json', 'ngrok-skip-browser-warning': 'true' };
